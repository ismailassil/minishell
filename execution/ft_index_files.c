...
void	ft_index_infiles(int *i, t_token *tok, t_cont *cont, t_file *h)
{
	cont->infile_index[*i] = tok->next->findex;
	cont->inf_is_amb[*i] = h->status;
	h = h->next;
	(*i)++;
}
	...
	while (i.cont != NULL)
	{
		...
		while (i.tok != NULL && i.tok->type != PIPE)
		{
			if (i.tok->type == APPEND || i.tok->type == OUTFILE)
				ft_index_outfiles(&i.j, i.tok, i.cont, i.h);
