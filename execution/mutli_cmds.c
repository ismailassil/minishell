...
/*   Updated: 2024/03/31 20:30:30 by iassil           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_pipe_to_next_child(t_info *info)
{
	if (info->fd.outfile == 1)
	{
		ft_syscall(dup2(info->pipe[1], STDOUT_FILENO), "msh: dup2 (PIPE)");
		ft_syscall(close(info->pipe[1]), "msh: close");
	}
	else
	{
...
static void	ft_child_process(t_cont *cont, t_struct *strp, t_info *info)
{
	...
	if (info->fd.infile != 0)
		ft_syscall(close(info->fd.infile), "msh: close");
	...
	if (execve(exec.cmd_path, exec.argv, exec.envp) == -1)
		(free(exec.argv), free(exec.envp), ft_error("msh: "),
...
void	ft_execute_child(t_cont *cont, t_struct *strp, t_info *info)
{
	...
	{
		ft_syscall(pipe(info->pipe), "msh: pipe");
		info->id[info->i] = fork();
